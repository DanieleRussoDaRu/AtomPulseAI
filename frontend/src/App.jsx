...
export default function App() {
  ...
  // 🌟 CORRETTO: Nome coerente e inizializzato come array []
  const [availablePartitions, setAvailablePartitions] = useState([]);

  const fetchPartitions = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/partitions`);
      const parts = response.data.partitions || [];
      
      // Salva l'elenco completo nell'array dello stato
      setAvailablePartitions(parts);
      
      // 🌟 CORRETTO: Se c'è almeno una partizione, imposta lo stato 'partition' di default
      if (parts.length > 0 && !partition) {
        setPartition(parts[0]);
      }
    } catch (error) {
      console.error("Error loading partitions list:", error);
    }
  };
  ...
  // ... tutto il resto del codice (fetchDocuments, handleSave e il return JSX) va benissimo così com'è!
  ...
  return (
        ...
        <div style={styles.filterRow}>
          ...
          >
            {availablePartitions.length === 0 ? (
              <option value="">No partitions available</option>
            ) : (
                ...
                {documents.length === 0 ? (
                  ...
                  documents.map((doc) => (
                    <tr key={doc._id} style={styles.trRow}>
                      ...
                      <td style={styles.td}>{doc.ap_code || 'Empty'}</td>
                      <td style={styles.td}>
    ...
    <select 
      ...
      style={styles.selectFilter}
    >
      {/* Opzione segnaposto: valore vuoto "", disabilitata così non è riselezionabile */}
...
// Inline CSS Styles
const styles = {
  container: { maxWidth: '95%', margin: '30px auto', padding: '0 20px', fontFamily: 'Segoe UI, Roboto, sans-serif' },
  card: { backgroundColor: '#f3f1f1', padding: '20px', borderRadius: '8px', border: '1px solid #e0e0e0', marginBottom: '20px' },
  ...
  selectFilter: { padding: '8px 12px', borderRadius: '4px', border: '1px solid #0070f3', fontSize: '14px', backgroundColor: 'white', color: '#333', cursor: 'pointer', minWidth: '220px'}
};